...
def get_accuracy(model, dataloader, device):
    ...
    for input_ids, input_mask, segment_ids, label_ids in tqdm(dataloader):
        ...
        with torch.no_grad():
            tmp_loss, logits = model(input_ids, segment_ids, input_mask, label_ids, return_logits=True)
        ...
        tmp_accuracy = accuracy(logits, label_ids.reshape(-1))
    ...
    return loss, accuracy, logits_all


def get_model_input_data(processor, n_class, args, tokenizer, mode='train'):
    """mode is in ['train', 'dev', 'test']"""
    label_list = processor.get_labels()
    examples = eval('processor.get_{}_examples()'.format(mode))
    feature_dir = os.path.join(args.data_dir, '{0}_features{1}.pkl'.format(mode, args.max_seq_length))
    ...
    dataloader = DataLoader(data, sampler=sampler, batch_size=eval('args.{}_batch_size'.format(mode)))
    return dataloader, len(examples)


def persistent_model_results(result, logits_all, output_dir, mode='dev'):
    output_eval_file = os.path.join(output_dir, "results_{}.txt".format(mode))
    with open(output_eval_file, "w") as writer:
        logger.info("***** {} results *****".format(mode))
        for key in sorted(result.keys()):
            logger.info("  %s = %s", key, str(result[key]))
            writer.write("%s = %s\n" % (key, str(result[key])))
    output_eval_file = os.path.join(output_dir, "logits_{}.txt".format(mode.capitalize()))  # 首字母大写
...
def main ():
    ...
    # 获得eval数据
    if args.do_eval:
        eval_dataloader, eval_examples = get_model_input_data(processor, n_class, args, tokenizer, mode='dev')
    ...
    if args.do_train:
        ...
        train_dataloader = get_model_input_data(processor, n_class, args, tokenizer, mode='train')
        ...
        for ie in range(int(args.num_train_epochs)):
            ...
            if args.do_eval:
                model.eval()
                eval_loss, eval_accuracy, logits_all = get_accuracy(model, eval_dataloader, device)
    ...
    if args.do_eval:
        ...
        model.eval()
        eval_loss, eval_accuracy = get_accuracy(model, eval_dataloader, device)

        result = {'eval_loss': eval_loss,
                  'eval_accuracy': eval_accuracy}

        persistent_model_results(result, logits_all, args.output_dir, mode='dev')

        test_dataloader, test_examples = get_model_input_data(processor, n_class, args, tokenizer, mode='test')
        logger.info("***** Running testing *****")
        logger.info("  Num examples = %d", len(test_examples))
        logger.info("  Batch size = %d", args.eval_batch_size)
        model.eval()
        test_loss, test_accuracy, logits_all = get_accuracy(model, test_dataloader, device)

        result = {'test_loss': test_loss,
                  'test_accuracy': test_accuracy}

        persistent_model_results(result, logits_all, args.output_dir, mode='test')
